...
import { AlertTriangle, Activity, Info, ShieldAlert, HeartPulse, FileWarning, BriefcaseMedical, Users, Server, ExternalLink, Lightbulb } from 'lucide-react';
...
export function TriageResult({ result }: { result: UHIBResult }) {
  ...
  return (
    <div className="w-full space-y-6">
      <div className={cn(
        "w-full rounded-2xl overflow-hidden shadow-sm border-2 transition-all duration-500",
        ...
        <div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              )}>Severity: {result.severity || 'Assessed'}</h2>
          ...
          <div className="flex flex-col items-start md:items-end bg-white dark:bg-zinc-900/50 p-4 rounded-xl border border-white/20 dark:border-zinc-700/30 shadow-sm shrink-0">
            ...
            <div className="flex items-end gap-1">
              <span className="text-4xl font-black text-zinc-800 dark:text-zinc-100 leading-none">
                {Math.round((result.confidence || 0) * 100)}<span className="text-2xl text-zinc-400">%</span>
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
        <div className="flex items-center gap-3 mb-4 border-b border-zinc-100 dark:border-zinc-800 pb-3">
           ...
           <h3 className="text-xs font-black text-zinc-500 dark:text-zinc-400 uppercase tracking-widest">Confidence Reason / Context</h3>
        </div>
        <p className="text-zinc-800 dark:text-zinc-200 text-lg leading-relaxed font-medium">
          {result.confidence_reason || 'Standard protocols engaged.'}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-6">
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm transition-all hover:shadow-md">
            ...
            <p className="text-2xl font-black text-zinc-900 dark:text-zinc-100 leading-tight">{result.condition}</p>
          </div>
          ...
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
            ...
            <ul className="space-y-3">
              {safeArray(result.required_resources).map((res, i) => (
            ...
            <div className="space-y-5">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {safeArray(result.data_sources).map((ds,i) => <span key={i} className="text-xs px-2.5 py-1 bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 rounded font-mono font-semibold">{ds}</span>)}
                </div>
              </div>
              <div>
                ...
                <ol className="list-decimal list-inside text-sm text-zinc-600 dark:text-zinc-400 font-medium space-y-1.5 marker:text-zinc-400">
                  {safeArray(result.processing_steps).map((ps,i) => <li key={i}>{ps}</li>)}
                </ol>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
            ...
            <ul className="space-y-3">
              {safeArray(result.risk_factors).map((risk, i) => (
                <li key={i} className="flex gap-3 items-start group">
          ...
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
            ...
            <ul className="space-y-4">
              {safeArray(result.similar_cases).map((cs, i) => (
          ...
          <div className="bg-indigo-50 dark:bg-indigo-900/10 p-6 rounded-2xl border border-indigo-200 dark:border-indigo-900/30 shadow-sm">
            ...
            <ul className="space-y-2">
               {safeArray(result.videos).length > 0 ? (
                 safeArray(result.videos).map((vid, i) => (
                   <li key={i}>
                  ...
                  <li className="text-sm text-zinc-500">No video references dynamically provided.</li>
               )}
            </ul>
          </div>
...
export function TriageSkeleton() {
  return (
    <div className="w-full space-y-6 animate-pulse">
      ...
      <div className="w-full h-24 rounded-2xl bg-zinc-200 dark:bg-zinc-800"></div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
